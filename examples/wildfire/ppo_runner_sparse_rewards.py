...
def cumulative_moe_reward(
    ...
    base_moe = 1.0 - (
        ...
        + MOE_WEIGHT * (casualties / CASUALTY_NORM)
    )
    penalized_moe = base_moe - 0.3 * propagation_factor
    return max(0.0, penalized_moe)
...
class WildfireHourlyEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def _compute_reward_and_info(
        ...
        if self.sim is not None:
            propagation_factor = (
                0.0 if self.sim.wildfire.fire_in_bounds else 1.0
            )
    ...
    def step(
        ...
        if terminated:
            ...
            final_reward = cumulative_moe_reward(
                metrics.burnt_area,
                metrics.cost,
                metrics.emissions,
                metrics.casualties,
            ...
            info["episode_summary"] = {
                ...
                * (self.decision_interval.total_seconds() / 60.0),
                ...
                "moe_cumulative_reward": final_reward,
                "propagation_factor": propagation_factor,
            }
...
def run_policy_evaluation(
    ...
    *,
    ...
    propagation_factor = episode_summary.get(
        "propagation_factor", last_info.get("propagation_factor", 0.0)
    )
    summary["propagation_factor"] = propagation_factor
...
def _select_batch_size(rollout_size: int) -> int:
    """Return a PPO batch size that divides the rollout buffer size."""
    preferred = 64
    if rollout_size % preferred == 0:
        return preferred

    # Try common divisors in descending order so we keep batches reasonably sized.
    for candidate in (128, 96, 72, 64, 48, 36, 32, 24, 16):
...
def main() -> None:
    ...
    batch_size = _select_batch_size(rollout_steps)
