...
COST_WEIGHT_NAMES: tuple[str, ...] = (
    "distance_cost_weight",
    "vip_cost_weight",
    "priority_cost_weight",
    "vegetation_cost_weight",
    "topography_cost_weight",
)


def _scenario_cost_weights(
    parameters: WildfireParameters,
) -> tuple[tuple[str, dict[str, float]], ...]:
    """Firefront-selection weights each aircraft type will actually use.

    One row per `agents` entry, in the order the firefighter model assigns
    `ac_type_id`. A scenario with no `weights` key reports the scenario-wide
    weights against every type, which is exactly what those aircraft use.
    """
    rows: list[tuple[str, dict[str, float]]] = []
    for ac_type_id, agent in enumerate(parameters.agents):
        weights = parameters.firefront_cost_weights(ac_type_id)
        label = str(agent.file_name or agent.output_id_name or f"type {ac_type_id}")
        rows.append(
            (label, {name: float(getattr(weights, name)) for name in COST_WEIGHT_NAMES})
        )
    return tuple(rows)


def _print_scenario_cost_weights(
    scenario_path: Path,
    parameters: WildfireParameters,
) -> None:
    """Record the per-type weights in the run log."""
    def pretty(weights: dict[str, float]) -> str:
        return ", ".join(
            f"{name.replace('_cost_weight', '')}={value:g}"
            for name, value in weights.items()
        )

    rows = _scenario_cost_weights(parameters)
    head = f"Firefront cost weights [{scenario_path.name}]:"
    if not rows:
        print(f"{head} no aircraft types")
    elif not parameters.weights:
        print(f"{head} scenario-wide {pretty(rows[0][1])}")
    else:
        print(f"{head} per aircraft type")
        for ac_type_id, (label, weights) in enumerate(rows):
            print(f"  ac_type_id={ac_type_id} {label}: {pretty(weights)}")
...
class WildfireHourlyEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def _load_parameters(self, scenario_path: Path) -> WildfireParameters:
        ...
        # Per-type weights are matched to `agents` by position. The scenario
        # model checks that at load, but the fleet is later re-stamped through
        # `model_copy`, which does not re-run validators - so re-check here,
        # where the scenario file can still be named in the error.
        if params.weights and len(params.weights) != len(params.agents):
            raise ValueError(
                f"{scenario_path.name}: `weights` has {len(params.weights)} "
                f"entries but the scenario defines {len(params.agents)} "
                "aircraft type(s); give exactly one weight set per entry of "
...
def main() -> None:
    ...
    for _weights_path in (
        switch_scenario_paths if use_switch_scenario and switch_scenario_paths
        else (scenario_path,)
    ):
        _TerrainParametersCache.metadata = {}
        _print_scenario_cost_weights(
            _weights_path,
            WildfireParameters.model_validate_json(
